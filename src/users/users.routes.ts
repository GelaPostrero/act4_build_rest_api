...
userRouter.put("/user/:id", async (req : Request, res : Response): Promise<void> =>  {
    try {
        ...
        if (!username || !email || !password ) {
            res.status(StatusCodes.UNAUTHORIZED).json({error : `Please provide all the required parameters..`})
            return;
        }

        if (!getUser) {
            res.status(StatusCodes.NOT_FOUND).json({error : `No user with the id ${req.params.id}`})
            return;
        }
